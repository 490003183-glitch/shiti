...
[**下载拾屉 0.3.11 · DMG 安装包**](https://github.com/490003183-glitch/shiti/releases/download/v0.3.11/Shiti-0.3.11-universal.dmg) · [全部发行版本](https://github.com/490003183-glitch/shiti/releases)
...
修复中文输入时拼音组合被界面刷新覆盖、候选框中途消失的问题。便签编辑改为显式管理的原生 NSTextView：输入法组合期间保留原生编辑内容，不用已保存的文字覆盖；选字确认后才同步便签并自动保存。组合期间按键交给输入法，Esc 不直接收起面板，自定义全选暂不介入；候选框点击不作为普通外部点击收起。保留文字撤销/重做、正文优先全选与收起保存。未新增轮询或常驻定时器。
...
新增“更多 → 备份与恢复…”：选择备份后确认恢复全部便签、快捷入口及排列。恢复前先保存当前内容并保留一份备份；如果无法保留当前文件，恢复会中止。

拖动位移以固定画布为参照，修复按钮自身坐标随移动变化造成的跟手偏差。拖动中不吸附、不添加位移动画，松手后才吸附隐藏网格。

快捷按钮支持自由拖动摆放，松手后吸附 16 点隐藏网格并保存位置。已摆放的按钮不随搜索或窗口缩放重新排序；空间不够时可滚动访问。目标位置被占用时向下寻找空位。旧数据无需手动迁移，新入口放入空位，原文件不受影响。

修复从访达拖入文件夹时面板提前收起：在外部按住左键时保持展开，松开后才判断是否收起；拖入面板内时继续接收快捷入口。普通外部左键点击在松开时收起，右键行为不变，无需提前固定面板。
...
- 按住按钮拖动即可自行摆放；虚线框显示松手后的吸附位置，重启后保留位置。
- 拖动排列后按 **⌘Z** 撤销，**⇧⌘Z** 重做。便签编辑区保留文字撤销操作。
...
- `Backups/`：自动备份，最多 3 份、文件内容合计不超过 30 MiB。已有数据发生修改时，最多每 5 分钟备份一次写入前的内容，不设置常驻备份定时器。相同内容不重复备份，旧备份按数量和容量轮换。恢复前的额外备份也计入同一限额。

右上角“更多 → 备份与恢复…”会显示可恢复备份的日期、便签数、快捷入口数和大小。恢复需再次确认，原有编辑撤销记录会清空。损坏的主文件在恢复前也会保留，但不会作为有效备份列出。单份文件超过 30 MiB 时不自动备份，普通保存仍可继续；菜单会显示原因，请通过数据目录手动备份。

可以从右上角更多菜单打开此目录。退出应用后完整复制此目录即可备份。数据无法解码时暂停写入，保护已有内容。

便签以明文保存在本机，应用自身不提供加密。不要将个人 `shelf.json`、`Backups/`、真实便签截图或文件访问书签上传到仓库或 Issue。

自动保存会原子替换同一个 `shelf.json`，不会每次输入都增加备份；快捷访问仅保存引用，不缓存目标文件。除上述受限备份外，磁盘占用主要随保留的便签内容与快捷入口数量增长。删除的非空便签保留在系统废纸篓，清空废纸篓后释放空间。删除失败时暂存的文本仍保留以免丢失。
